...
void main() {
  ...
  group('CancellableHttpLink - Underlying HTTP abort verification', () {
    test(
        'cancellation aborts the underlying HTTP connection '
        '(server sees client disconnect)', () async {
      // This test proves the HTTP request is truly cancelled at network level.
      // The server delays its response long enough for the client to cancel,
      // then tries to write the response. If the client socket was aborted,
      // the server write will either fail or the response will never arrive.
      final abortServer = await HttpServer.bind('localhost', 0);
      final abortServerUrl = 'http://localhost:${abortServer.port}/graphql';

      final serverSawRequest = Completer<void>();
      final serverTriedResponding = Completer<bool>();

      abortServer.listen((request) async {
        serverSawRequest.complete();

        // Wait long enough for the client to cancel
        await Future<void>.delayed(const Duration(seconds: 2));

        try {
          request.response.statusCode = 200;
          request.response.headers.contentType = ContentType.json;
          request.response.write('{"data": {"test": "should not arrive"}}');
          await request.response.close();
          serverTriedResponding.complete(true); // wrote without error
        } catch (_) {
          serverTriedResponding.complete(false); // write failed (aborted)
        }
      });

      try {
        final link = CancellableHttpLink(abortServerUrl);
        final client = GraphQLClient(
          cache: GraphQLCache(),
          link: link,
        );
        ...
        // Now cancel — the request is in-flight
        cancellationToken.cancel();

        // The result should surface a CancelledException
        final result = await resultFuture;
        expect(result.hasException, isTrue);
        expect(result.exception!.linkException, isA<CancelledException>());

        // Verify the server side: either the write failed (connection reset)
        // or it didn't complete normally before the client disconnected.
        // On IO the server may still manage to write after the client closes,
        // but the client must NOT have received the 200 with data.
        expect(result.data, isNull,
            reason: 'Cancelled request must not deliver data');

        cancellationToken.dispose();
        await link.dispose();
      } finally {
        // Ensure the server-side future settles before closing
        await serverTriedResponding.future
            .timeout(const Duration(seconds: 5), onTimeout: () => false);
        await abortServer.close(force: true);
      }
    });

    test('non-cancelled request does receive data', () async {
      // Control test: same setup but without cancellation ⇒ data arrives.
      final controlServer = await HttpServer.bind('localhost', 0);
      final controlUrl = 'http://localhost:${controlServer.port}/graphql';

      controlServer.listen((request) async {
        await Future<void>.delayed(const Duration(milliseconds: 50));
        request.response.statusCode = 200;
        request.response.headers.contentType = ContentType.json;
        request.response.write('{"data": {"test": "arrived"}}');
        await request.response.close();
      });

      try {
        final link = CancellableHttpLink(controlUrl);
        final client = GraphQLClient(
          cache: GraphQLCache(),
          link: link,
        );
        ...
        expect(result.hasException, isFalse);
        expect(result.data, equals({'test': 'arrived'}));
